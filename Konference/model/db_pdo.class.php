...
class db_pdo
{
	...
	public function DBUpdate($table_name,$item, $where_array)
	{
		...
		if ($where_array != null)
			foreach ($where_array as $index => $item2)
			{
				// pridat AND
				if ($where_pom != "") $where_pom .= "AND ";
				...
				if (key_exists("value", $item2))
					$value_pom = "?"; 						// budu to navazovat
				else if (key_exists("value_mysql", $item2))
					$value_pom = $item2["value_mysql"]; 		// je to systemove, vlozit rovnou - POZOR na SQL injection, tady to muze projit
		...
		if ($item != null) {
			...
			foreach ($where_array as $row)
			{
				...
				$statement->bindValue($bind_param_number, $row['value']);  // vzdy musim dat value, abych si nesparoval promennou (to nechci)
				$bind_param_number ++;
			}
		}
		// 4) provest dotaz
